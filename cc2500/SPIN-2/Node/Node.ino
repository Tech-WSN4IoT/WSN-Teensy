...
double S3TO                =  3000;  // State 3 Iteration Time Out in ms
...
//--------------------------Hard coded packet information---------------------
//NOTICE: THIS IS NOT GOOD PRACTICE, GO BACK TO THE CONSTRUCTOR WHEN THERE IS MORE TIME

Packet gPacket; //General use packet
Packet eventPacket; //General packet for node events

//char gPacket[4] = {0x00, 0x00, 0x00, 0x00};
//char eventPacket[4] = {0x00, 0x00, 0x00, 0x00}; 
...
void loop() {
  ...
  if (!isGate) { //States S3 and S4 are locked only to nodes. Gates only do states S1 and S2 and report all data through serial through puts.
    ...
    if (state == 5) {
      ...
      /*
      Serial.println(eventPacket.length(),HEX); 
      Serial.println(eventPacket.nodeName(),HEX); 
      Serial.println(eventPacket.type(),HEX); 
      Serial.println(eventPacket.data(),HEX); 
      */
      delay(100);
    }
  } //!isGate
  //---------------------------------GATEWAY OUTPUT -------------------------------------------o
    ...
    if (isInteresting(eventPacket)) {
      Serial.println("GATEWAY RECEIVE-----------------------O");
      Serial.print("From Node: ");
      Serial.println(id, HEX);
      Serial.print(int(data));
      Serial.println(" degrees temp");
      Serial.println("--------------------------------------");
      eventPacket.setnodeName(0x00); 
      gPacket.setnodeName(0x00); 
    }
  }
} //END LOOP -----------------------------------------------------------------------------------o
